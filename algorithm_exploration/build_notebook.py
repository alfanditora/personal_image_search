 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "# 5. Helper untuk memuat wajah (face ROIs) secara efisien (menggunakan bboxes dari cache jika ada)\n",
    "def get_face_rois(img_path, detector, arcface_cache):\n",
    "    img_name = os.path.basename(img_path)\n",
    "    # Jika bboxes terdeteksi ada di cache ArcFace, crop langsung untuk efisiensi\n",
    "    if img_name in arcface_cache:\n",
    "        rois = []\n",
    "        img = cv2.imread(img_path)\n",
    "        if img is None:\n",
    "            return []\n",
    "        h_img, w_img = img.shape[:2]\n",
    "        for entry in arcface_cache[img_name]:\n",
    "            bbox = entry[\"bbox\"]\n",
    "            x, y, w, h = bbox\n",
    ...
    "            if face_roi.size == 0:\n",
    "                continue\n",
    "            aligned_face = detector.align_face(face_roi)\n",
    "            resized_face = cv2.resize(aligned_face, (112, 112), interpolation=cv2.INTER_AREA)\n",
    "            rois.append({\"cropped_face\": resized_face, \"bbox\": bbox})\n",
    "        return rois\n",
    "    # Fallback jika belum ada cache\n",
    ...
    "    rois = get_face_rois(img_path, detector, preloaded_caches[\"ArcFace\"])\n",
    "    for roi in rois:\n",
    ...
    "            \"bbox\": roi[\"bbox\"]\n",
    "        })\n",
    "print(f\"Berhasil mengumpulkan {len(dataset_faces)} wajah dari {len(test_images)} gambar uji dalam {time.time() - t0:.2f} detik.\")\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "# 2. Ekstraksi Fitur Dataset (Mengambil dari cache, atau extract jika tidak ada)\n",
    "dataset_feats_list = []\n",
    "for face_meta in dataset_faces:\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "# 2. Plot Waktu Komputasi Total (Feature Extraction + Inference/Matching)\n",
    "plt.subplot(1, 3, 2)\n",
    "total_time = df_results['Feature Extraction Time (s)'] + df_results['Inference Time (s)']\n",
    "bars2 = plt.bar(df_results['Model'], total_time, color=colors[:len(df_results)])\n",
    "plt.title('Total Waktu Komputasi (detik)', fontsize=12, fontweight='bold')\n",
    "plt.xticks(rotation=45, ha='right', fontsize=9)\n",
    "plt.ylabel('Waktu (detik)')\n",
    "for bar in bars2:\n",
    "    yval = bar.get_height()\n",
    "    plt.text(bar.get_x() + bar.get_width()/2.0, yval + (yval * 0.02) + 0.05, f\"{yval:.2f}s\", ha='center', va='bottom', fontsize=9)\n",
    "\n",
    "# 3. Plot Memory Usage\n",
    "plt.subplot(1, 3, 3)\n",
